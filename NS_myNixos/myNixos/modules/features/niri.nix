    ...
    packages.myNiri = inputs.wrapper-modules.wrappers.niri.wrap {
      ...
      settings = {
        ...
        binds = {
          ...
          # Helium isn't in nixpkgs (see modules/features/helium.nix),
          # so there's no `pkgs.helium` to call lib.getExe on here.
          # It's on PATH via environment.systemPackages though, so
          # the bare binary name works fine.
          "Mod+H" = { spawn = [ "helium" ]; };

          # yazi in a floating-ish terminal window
          "Mod+Y" = { spawn = [ (lib.getExe pkgs.ghostty) "-e" "yazi" ]; };
